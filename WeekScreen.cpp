...
//		if(dwConfirm && MessageBox(hWnd, TEXT("Delete Highlighted Item?"), TEXT("Delete"),MB_OKCANCEL) != IDOK)
		if(dwConfirm && MessageBox(hWnd, TEXT("Ê²×²Ä‚³‚ê‚½±²ÃÑ‚ðíœ‚µ‚Ü‚·‚©?"), TEXT("Delete"),MB_OKCANCEL) != IDOK)	// by uminchu
			break;
		OpenClipboard(hWnd);
		EmptyClipboard();
		ClipBuf = 0;
...
//			DlgResult = MessageBox(hWnd, TEXT("Delete All Item?"), TEXT("Repeat Delete"),MB_YESNOCANCEL);
			DlgResult = MessageBox(hWnd, TEXT("‘S‚Ä‚Ì±²ÃÑ‚ðíœ‚µ‚Ü‚·‚©?"), TEXT("Repeat Delete"),MB_YESNOCANCEL);	// by uminchu
