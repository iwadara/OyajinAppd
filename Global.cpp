...
LPCTSTR		szVerNum =			TEXT("0.43");		// szVerNum Number TEXT	[iwad]
...
LPCTSTR		DayName[] = {	// DayName;
				TEXT("ì˙"), TEXT("åé"), TEXT("âŒ"),TEXT("êÖ"), TEXT("ñÿ"), TEXT("ã‡"),TEXT("ìy")}; // by uminchu
//				TEXT("Sunday"), TEXT("Monday"), TEXT("Tuesday"),TEXT("Wednesday"), TEXT("Thursday"), TEXT("Friday"),TEXT("Saturday")};

LPCTSTR		MonthName[] = {	// DayName;
				TEXT("ÇPåé"), TEXT("ÇQåé"), TEXT("ÇRåé"),TEXT("ÇSåé"), TEXT("ÇTåé"), TEXT("ÇUåé"),TEXT("ÇVåé"), TEXT("ÇWåé"), TEXT("ÇXåé"),TEXT("10åé"), TEXT("11åé"), TEXT("12åé")} ;	// by uminchu
...
DWORD		dwHourMode = 1;	// Hour Disp Mode (0:12H 1:24H) [iwad] ÉfÉtÉHÉãÉgÇÕ24éûä‘ï\ãL
...
DWORD		dwAddinMenu;	// [iwad] ägí£ÉÅÉjÉÖÅ[óp
...
LPCTSTR	ShortDayName[] = {TEXT("ì˙"), TEXT("åé"), TEXT("âŒ"), TEXT("êÖ"),TEXT("ñÿ"), TEXT("ã‡"), TEXT("ìy")};	// by uminchu
...
LPCTSTR	RepeatName[] = {	// Repeat Status String
	TEXT("None"), TEXT("ñàì˙"), TEXT("ñàèT"), TEXT("ñàåé"), TEXT("ñàîN")};	// by iwad
...
FIELD	MainField[] ={
	...
	FLGROUPBOX,	0,	"ï\é¶",				3,68,249,25,// Group Box
...
//
FIELD	BackField[] ={
	FLSTATIC,	0,	"ì‡óe(&R)",			5,7,35,12,	// Title String Caption
	FLSTRING,	64,(DWORD *)BackTitle,	40,4,205,13,// Title String
	FLSTATIC,	0,	"äJénéûçè(&T)",		5,23,35,12,	// Start Time Caption
	FLTIME,		7,	&BackStartTime,		40,21,35,12,// Start Time Data
	FLSTATIC,	0,	"èIóπéûçè(&I)",		5,39,35,12,	// End Time Caption
	FLTIME,		7,	NULL,				40,37,35,12,// End Time 2
	FLSTATIC,	0,	"èÍèä(&C)",			5,55,35,12,	// Location Caption
	FLSTRING,	32,(DWORD *)BackLocation,	40,53,120,13,// Location
	FLCHECK,	1,	&BackEnable,		85,30,35,12,// Enable
	FLSTATIC,	0,	"óLå¯(&B)",			85,30,35,12,// Enable Caption
	FLSTATIC,	0,	"ï™ëO(&L)",			136,33,20,12,// Lead Caption
	FLBYTE,		2,	&BackLeadTime,		120,30,15,12,// Lead Time
	FLGROUPBOX,	0,	"±◊∞—",				80,17,80,32,// Group Box
	FLSTATIC,	0,	"“”óì(&N)",			5,95,15,24,	// Note Caption
	FLMULTIEDIT,4096,NULL,				20,95,185,32,// Note
	FLSTATIC,	0,	"ì˙ïtïœçX(&D)",		165,23,40,12,// Date Caption
	FLDATE,		10,	NULL,				205,21,40,12,// Original Date
	FLSTATIC,	0,	"òAë±ì˙êî(&U)",		165,39,65,12,// Cons Days Caption
	FLBYTE,		2,	NULL,				230,37,15,12,// Cons Days
	FLSTATIC,	0,	"åJï‘ÇµΩ√∞¿Ω(&S)",	165,55,40,12,// Repeat Status Caption
	FLREPEAT,	14,	NULL,				215,53,30,12,// Repeat Status
	FLCHECK,	1,	NULL,				10,77,40,12,// Week View CheckBox
	FLSTATIC,	0,	"åéï\é¶(&M)",		10,77,40,12,// Week View Caption
	FLCHECK,	2,	NULL,				50,77,40,12,// Month View
	FLSTATIC,	0,	"èTï\é¶(&W)",		50,77,40,12,// Month View Caption
	FLCHECK,	4,	NULL,				90,77,35,12,// Month View
	FLSTATIC,	0,	"ì˙ï\é¶(&A)",		90,77,35,12,// Month View Caption
	FLCHECK,	8,	NULL,				127,77,35,12,// Month View
	FLSTATIC,	0,	"êFïtÇ´(&G)",		127,77,35,12,// Month View Caption
	...
	FLSTATIC,	0,	"Ãﬂ◊≤Õﬁ∞ƒ(&P)",		200,77,40,12,// Month View Caption
	FLGROUPBOX,	0,	"ï\é¶",				5,66,240,25,// Group Box
...
*/
// by uminchu
LPCTSTR	szTips[] = {NULL, TEXT("îºîNï\é¶"),TEXT("åéä‘ï\é¶"), TEXT("èTä‘ï\é¶"),TEXT("ì˙ï\é¶"), TEXT("ì¸óÕâÊñ "),
					TEXT("ToDo"),
					TEXT(" ﬂ∞ø≈Ÿ”∞ƒﬁ"),
					TEXT("Ωπºﬁ≠∞ŸÇÃçÌèú"),
#ifdef	_WIN32_WCE
...
LPCTSTR		RegTable[][4] = {
	...
	TEXT("FileMode"),	(TCHAR *)1,	(TCHAR *)&dwFileMode,	NULL,	// ÉfÅ[É^îjâÛñ‚ëËëŒçÙÅAÉLÉÉÉbÉVÉÖégópÇã≠êßìIÇ…óLå¯Ç…Ç∑ÇÈ(iwad 2007.01.22èCê≥)
...
//	TEXT("SmallFontSize"),(TCHAR *)1,(TCHAR *)&dwSmallFontSize,	NULL,
	...
	TEXT("AddinMenu"),	(TCHAR *)1, (TCHAR *)&dwAddinMenu,	NULL,	// [iwad] ägí£ÉÅÉjÉÖÅ[óp
...
//	TEXT("BigFontName"),(TCHAR *)0,	szBigFontName,		TEXT(" é©ìÆ"),	// by uminchu
	TEXT("BigFontName"),(TCHAR *)0,	szBigFontName,		TEXT("ÇlÇr ÇoÉSÉVÉbÉN"),	// [iwad]
//	TEXT("MidFontName"),(TCHAR *)0,	szMidFontName,		TEXT(" Auto"),
//	TEXT("MidFontName"),(TCHAR *)0,	szMidFontName,		TEXT(" é©ìÆ"),	// by uminchu
	TEXT("MidFontName"),(TCHAR *)0,	szMidFontName,		TEXT("ÇlÇr ÇoÉSÉVÉbÉN"),	// [iwad]
//	TEXT("SmallFontName"),(TCHAR *)0,szSmallFontName,	TEXT(" Auto"),
//	TEXT("SmallFontName"),(TCHAR *)0,szSmallFontName,	TEXT(" é©ìÆ"),	// by uminchu
	TEXT("SmallFontName"),(TCHAR *)0,szSmallFontName,	TEXT("ÇlÇr ÇoÉSÉVÉbÉN"),	// [iwad]
